...
if (isset($_POST['simpan'])) {
    if ($_GET['b'] == 'tambah') { 
        ...
        if(!in_array($ext,$ekstensi) ) {
            ...
            echo "<script>
                alert('Data gangguan dan hama berhasil disimpan');
                window.location.href='?p=gangguan';
                </script>";
        }
    ...
    else
    {
        ...
        if ($filename != null) {
            if(!in_array($ext,$ekstensi) ) {
                ...
                $xx = $rand.'_'.$filename;
                move_uploaded_file($_FILES['foto']['tmp_name'], 'style/img/'.$rand.'_'.$filename);
                mysqli_query($con," UPDATE gangguan set nama_gangguan = '$nama_penyakit',desk_gangguan = '$desk_penyakit', saran = '$saran_penyakit', gambar = '$xx', jenis = '$jenis' where idgangguan = '$id' ");
                echo "<script>
                    window.location.href='?p=gangguan&&gangguan=update';
                    </script>";
            }
        }
        else
        {
            mysqli_query($con," UPDATE gangguan set nama_gangguan = '$nama_penyakit',desk_gangguan = '$desk_penyakit', saran = '$saran_penyakit', jenis = '$jenis' where idgangguan = '$id' ");
             echo "<script>
                    window.location.href='?p=gangguan&&gangguan=update';
                    </script>";
        }
...
elseif ($_GET['b'] == 'update') {
    $id     = $_GET['id'];
    $sql    = mysqli_query($con," SELECT * FROM gangguan where idgangguan = '$id' ");
    ...
    <div class="container-fluid px-4">
        ...
        <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
            ...
            <div class="card-body row">
                ...
                <div class="col-md-10"><input type="text" value="<?=empty($d)?'':$d['nama_gangguan']?>" name="nama_penyakit" class="form-control" required="" ></div><br><br>

                <div class="col-md-2">Deskripsi Penyakit</div>
                <div class="col-md-10"><textarea class="ckeditor" id="desk_penyakit" name="desk_penyakit"><?=empty($d)?'':$d['desk_gangguan']?></textarea></div>
