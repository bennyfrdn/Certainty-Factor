 ...
 if (isset($_POST['simpan'])) {
    if ($_GET['b'] == 'tambah') {
        $nama       = $_POST['nama'];
        $nohp       = $_POST['nohp'];
        $cekadmin   = mysqli_query($con," SELECT * FROM user where nohp = '$nohp' ");
        ...
        echo "<script>
         ...
         mysqli_query($con," INSERT INTO user values ('','$nama','$nohp') ");
        echo "<script>
...
}else 
    {
        ...
        $nama       = $_POST['nama'];
        $nohp   = $_POST['nohp'];
        mysqli_query($con," UPDATE user set nama = '$nama', nohp = '$nohp' where iduser = '$id' ");
        echo "<script>
                window.location.href='?p=user_update&&user=update';
                </script>";
    }
